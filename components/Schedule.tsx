...
export default function Schedule() {
  ...
  return (
    <SectionWrapper id="schedule">
      <SectionHeading text="Event" accent="Schedule" subtitle="Three days of non-stop tech, culture, and competition" />

      <div className="max-w-4xl mx-auto">
        <div className="flex justify-center gap-3 mb-12">
          {days.map((day) => (
            <button key={day} onClick={() => setActiveDay(day)}
              className={`relative px-6 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 ${activeDay === day ? "text-white" : "text-[var(--clr-text-muted)] hover:text-white border border-white/10 hover:border-white/20"}`}>
              {activeDay === day && (
                <motion.div layoutId="schedule-tab" className="absolute inset-0 bg-[var(--clr-primary)] rounded-full -z-10" style={{ boxShadow: "var(--shadow-glow-md)" }} transition={{ type: "spring", duration: 0.5, bounce: 0.2 }} />
              )}
              {day}
            </button>
          ))}
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activeDay} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="relative">
            ...
            <div className="space-y-6">
              {(scheduleData[activeDay] || []).map((event, index) => (
                <motion.div key={`${activeDay}-${index}`} className="relative pl-14 group" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1, duration: 0.4 }}>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1.5">
                          ...
                          <span className="text-[var(--clr-primary)] text-sm font-semibold">{event.time}</span>
                        </div>
                        <h3 className="text-lg font-bold tracking-tight">{event.title}</h3>
                        {event.desc && <p className="text-[var(--clr-text-muted)] text-sm mt-1 leading-relaxed">{event.desc}</p>}
                      </div>
                      <ChevronRight size={18} className="text-[var(--clr-text-dim)] group-hover:text-[var(--clr-primary)] group-hover:translate-x-1 transition-all duration-300 flex-shrink-0 mt-1" />
                    </div>
                  </div>
                </motion.div>
              ))}
